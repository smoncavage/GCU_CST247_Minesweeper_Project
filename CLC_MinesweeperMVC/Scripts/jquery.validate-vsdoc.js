...
$.extend($.validator, {
	...
	methods: {
		...
		email: function(value, element) {
			...
			// Avoid catastrophic backtracking from complex RFC-style regex by using URL parsing.
			// Keep validation practical: require exactly one "@", non-empty parts, and no whitespace.
			if ( /\s/.test(value) || value.indexOf("@") === -1 || value.indexOf("@") !== value.lastIndexOf("@") ) {
				return false;
			}

			try {
				var parsed = new URL("mailto:" + value);
				var address = parsed.pathname;
